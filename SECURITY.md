...
`audit-root-storage` discloses only bounded, sanitized metadata. It does not read regular-file contents, symlink targets, xattrs, or nested mounts. Sensitive and non-conservative path components are rendered as `<redacted>`, and scanner failures use fixed generic messages rather than underlying OS exception text. The audit is progressive: each top-level subtree is bounded by its own entry budget and the whole run by a global entry budget and time budget, so an oversized subtree or exhausted global budget yields completed top-level results plus explicit `partial`, `excluded`, and `not_audited` markers rather than an all-or-nothing failure; when the global budget or deadline is hit before top-level enumeration completes, `not_audited` is reported as the sentinel `unbounded` because the number of unvisited subtrees is unknown. Incomplete subtrees are never claimed as exact sizes. The sudoers file carries the reviewed helper SHA-256 authority; authority, helper, trusted-path, traversal, or mount-map drift causes failure before audit output.

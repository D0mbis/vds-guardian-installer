...
- `dist/install-new.sh`: creates a new `guardian` account, accepts one public SSH key through `--public-key`, and stores a short-lived one-time proof supplied through `--enrollment-token`. The proof lets Hermes verify first contact without asking the user to compare SSH fingerprints and is removed after successful enrollment.

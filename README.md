...
- `audit-root-storage` — metadata-only allocated-space inventory of first- and second-level objects below `/root`, plus fixed cache, backup/archive, Git working-copy, log, and temporary-artifact totals. It never follows symlinks, opens regular-file contents, or crosses nested mount points; sensitive or unusual name components are redacted. Output is buffered until integrity, mount-map, traversal, and resource-limit checks complete. In deterministic traversal order, hard-linked regular files are charged once across reported object totals and once per applicable category;
- `audit-storage`
- `audit-services`
- `audit-security`
- `verify-health`

...
- `audit-root-storage` — progressive, metadata-only allocated-space inventory of first-level objects below `/root`. Each top-level subtree is scanned within its own entry budget and the whole run within a global entry budget and a fixed time budget; when a subtree or the global budget is exhausted the helper still returns the completed top-level results as a bounded top-64 summary (plus fixed cache, backup/archive, Git working-copy, log, and temporary-artifact totals) and explicitly marks every partial, mount-excluded, and not-audited subtree instead of failing the entire audit; if the global budget or deadline is hit before top-level enumeration completes, `not_audited` is reported as the literal sentinel `unbounded` because the number of unvisited subtrees is unknown. Incomplete subtrees and category totals use `size_lower_bound`, never an exact `size`; if several limits apply, the deterministic `reason` field preserves each cause. It never follows symlinks, opens regular-file contents, or crosses nested mount points; sensitive or unusual name components are redacted. Output is buffered until integrity, mount-map, traversal, and resource-limit checks complete. In deterministic traversal order, hard-linked regular files are charged once across reported object totals and once per applicable category;
- `audit-storage`
- `audit-services`
- `audit-security`
- `verify-health`
...
- `dist/upgrade-existing.sh`: fail-closed, no-argument root upgrade for hosts that have one of the exact previously published helper/sudoers pairs supported by the artifact, including the exact deployed `304d084` twelve-action helper/sudoers pair. It validates the fixed parent/leaf ownership boundary, serializes its own runs with a fail-fast lock under `/run`, rejects metadata or content drift, stages fixed-path replacements, and verifies rollback of both files against the selected exact baseline. It is idempotent only for the exact current files and does not change SSH, accounts, groups, Docker, services, or firewall state. The lock serializes this upgrader in the ordinary local administrative model; it is not a claim of protection from a malicious concurrent root process.

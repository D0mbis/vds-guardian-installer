...
- `audit-root-storage` — progressive, metadata-only allocated-space inventory of first-level objects below `/root`. Each top-level subtree is scanned within its own entry budget and the whole run within a global entry budget and a fixed time budget; when a subtree or the global budget is exhausted the helper still returns the completed top-level results as a bounded top-64 summary (plus fixed cache, backup/archive, Git working-copy, log, and temporary-artifact totals) and explicitly marks every partial, mount-excluded, and not-audited subtree instead of failing the entire audit; if the global budget or deadline is hit before top-level enumeration completes, `not_audited` is reported as the literal sentinel `unbounded` because the number of unvisited subtrees is unknown. Incomplete subtrees are never presented with exact sizes. It never follows symlinks, opens regular-file contents, or crosses nested mount points; sensitive or unusual name components are redacted. Output is buffered until integrity, mount-map, traversal, and resource-limit checks complete. In deterministic traversal order, hard-linked regular files are charged once across reported object totals and once per applicable category;
- `audit-storage`
- `audit-services`
- `audit-security`
- `verify-health`
